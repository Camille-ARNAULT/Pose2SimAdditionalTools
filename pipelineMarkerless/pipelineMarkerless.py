    ...
    ***
    ...
    #Si dossier calib existant vérifier qu'il contient la calibration, si non prévenir l'utilisateur (plus worth de juste déposer les vidéos dans le working directory si calib pas faite du coup)          
        ...
        for file in os.listdir(calibFolderPath) : 
            if not any(s in file for s in ["calib", "Calib"]):
                os.system("WARNING - Aucun fichier .toml de calibration retrouvé.")
            else :
                userCalib = True
    ...
    if nbtrials%1 == 0 :
        ...
        #Création des donssiers trials et sous-dossiers videos_raw
...
def synchronisation(curTrial=[],mosaicSyncControl=False,plotSyncControl=False) :
